...
const assert = require("assert");
  ...
  describe("Lottery Contract", () => {
    ...
    it("tests multiple entry", async ()=>{
      ...
      it('pickWinner test', async ()=>{
          try{
              await lottery_instance.methods.pickWinner.send({
                  from: ts[1],
                  value: 0
              });
              assert(false);
    ...
    it('est for the reset',async ()=>{
        await lottery_instance.methods.enter().send({
          from: ats[0],
          value: web3.utils.toWei('10','ether')
        });
        const initialBalance=await web3.eth.getBalance(ts[0]);
        await lottery.methods.pickWinner().send({from: ts[0]});
        const finalBalance= await web3.eth.getBalance(ts[0]);
